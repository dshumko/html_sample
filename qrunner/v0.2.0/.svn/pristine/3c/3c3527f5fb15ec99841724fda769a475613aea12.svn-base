...
test("is3D", function() {
    try {
        ok(programme.is3D !== undefined, "is exists");
    } catch (e) {
        ok (false, "not exists");
    }
});
...
try {
	avVideoComponent = avControl.getComponents(COMPONENT_TYPE_VIDEO);
} catch (e) {
...
try {
	avAudioComponent = avControl.getComponents(COMPONENT_TYPE_AUDIO);
} catch (e) {
...
try {
	avSubtitleComponent = avControl.getComponents(COMPONENT_TYPE_SUBTITLE);
} catch (e) {
